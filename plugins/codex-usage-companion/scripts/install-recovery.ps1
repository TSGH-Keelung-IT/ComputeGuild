...
if (-not $SkipTaskRegistration) {
    ...
    $settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -Hidden -ExecutionTimeLimit ([TimeSpan]::Zero) -MultipleInstances IgnoreNew
    $principal = New-ScheduledTaskPrincipal -UserId $userId -LogonType Interactive -RunLevel Limited
    Register-ScheduledTask -TaskPath $taskPath -TaskName $taskName -Action $action -Trigger @($recoveryTrigger, $logonTrigger) -Settings $settings -Principal $principal -Description 'Hosts Codex Usage Companion and restores it when Codex Desktop is running.' -Force | Out-Null
