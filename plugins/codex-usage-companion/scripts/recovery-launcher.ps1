...
try {
    ...
    # Keep the resident inside the scheduled task's Job instead of asking it to
    # detach. Task Scheduler owns this launcher for the resident's full lifetime,
    # so Windows cannot tear down the companion when the launcher exits.
    $process = Start-Process -FilePath $selected.Path -WindowStyle Hidden -Wait -PassThru
